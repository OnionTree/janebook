...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/janebook/css/font-awesome.min.css"/>
    <link rel="stylesheet" type="text/css" href="/janebook/css/layui.css"/>
    ...
    <script type="text/javascript" src="/janebook/layui.js"></script>
    <script type="text/javascript" src="/janebook/js/wangEditor.min.js"></script>
    <script type="text/javascript" src="/janebook/js/ddsort.js"></script>
    ...
    <div class="container-fluid">
        <div class="row normal-mode">
            <div class="col-sm-2 col-md-2 aside">
                ...
                <ul id="classify-box" class="notebooks wrap">
...
<script>
    $(function(){
        ...
        classifyBox.html("");
        classifyBox.on("click", function(e){
            var _this = $(e.target);
            var id = null;
            if(!_this.hasClass("one-notebook item")){
                ...
                success:function(data){
                    ...
                    $.each(data, function(index, obj){
                        var new_html = '<li class="one-note" data-model="note" id="'+index+'">' +
                            ...
                            '</ul>' +
                            '</li>';
                        articleBox.append(new_html);
                    })
                }
            })
        }
        $.each(classifys, function(index, obj){
            var new_html = '<li class="one-notebook item" id="'+obj.id+'">' +
                '<a href="javascript:void(0)" class="notebook-name"><span>'+obj.classifyName+'</span></a>' +
                '<a href="javascript:void(0)" class="edit-notebook dropdown-toggle" data-toggle="dropdown"> <i class="fa fa-gear"></i> </a>' +
                '<ul class="dropdown-menu arrow-top">' +
                '<li> <a href="javascript:void(0)" class="rename-notebook" data-toggle="modal"> <i class="fa fa-pencil-square-o"></i> 修改文集名 </a> </li>' +
                '<li class="divider"></li>' +
...
<script type="text/javascript">
    ...
    $(function(middle){
        ...
        $(".normal-mode .middle .notes").on('click','.one-note',function(e){   //将文章名字 传递到右侧顶上input框
            ...
            if(index == null){
                $(".right .wantEditor .input-title").val("");
                editor.txt.html("")
            }else{
                $(".right .wantEditor .input-title").val(articles[index].title);
                editor.txt.html(articles[index].content)
            }
        })  
    });
    $(function(add_note){
